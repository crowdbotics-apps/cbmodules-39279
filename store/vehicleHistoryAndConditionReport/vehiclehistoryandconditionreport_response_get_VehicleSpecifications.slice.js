...
const vehiclehistoryandconditionreport_response_get_VehicleSpecificationsSlice = createSlice({
  ...
  reducers: {},
  extraReducers: {
    [vehiclehistoryandconditionreport_get_specs_read.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    },
    [vehiclehistoryandconditionreport_get_specs_read.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = [...state.entities.filter(record => record.id !== action.payload.id), action.payload];
        state.api.loading = "idle";
      }
    },
    [vehiclehistoryandconditionreport_get_specs_read.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
